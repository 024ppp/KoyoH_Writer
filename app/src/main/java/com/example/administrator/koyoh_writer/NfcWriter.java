...
public class NfcWriter {
    ...
    public boolean write(Context context, Intent intent, String text) {
    ...
    private static NdefMessage createNdefMessage(String text) {
        //言語コードをバイト化
        byte[] langBytes = Locale.JAPANESE.getLanguage().getBytes(Charset.forName("US-ASCII"));
        ...
        //ステータスバイト、言語バイト、テキストバイトをつないでペイロードを構築
        byte[] payload = concat(new byte[] { (byte) status },
                langBytes, textBytes);
        NdefRecord rec = new NdefRecord(NdefRecord.TNF_WELL_KNOWN,
     ...
     * @return
     */
    private static byte[] concat(byte[]... bytesArray) {
        byte[] tmp = null;
    ...
    private NdefRecord createActionRecord() {
        ...
        byte[] typeField = "ja".getBytes(Charset.forName("US-ASCII"));
